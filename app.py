...
Веб-интерфейс CodeMentor AI на Gradio. Запуск:
    python app.py
Откроется локально на http://127.0.0.1:7860.
...
if __name__ == "__main__":
    # В Google Colab нет доступа к localhost снаружи, поэтому там нужен
    # публичный туннель Gradio (share=True). Локально он не нужен и не
    # включён по умолчанию. Colab-ноутбук сам выставляет GRADIO_SHARE=true
    # перед запуском — см. notebooks/demo_colab.ipynb.
    share_enabled = os.environ.get("GRADIO_SHARE", "false").lower() == "true"
    demo.launch(
        share=share_enabled,
